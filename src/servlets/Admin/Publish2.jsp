	...
	<table >
       ...
       <TR>
		...
		<% 
         ...
         String CatName=new String(request.getParameter("SelectCategory"));
         int Cat_Id=0;
         	CatName.trim();
         	rs=stmt.executeQuery("select Category_id from Category where name='"
         	+CatName
         	+"'");
            while(rs.next())
	          {
		...
		<%
			...
			stmt=con.createStatement();
			rs=stmt.executeQuery("select name from test where test_id in(select test_id from test_category where category_id="
			+Cat_Id
			+")");
			out.println("<OPTION>");
			while(rs.next())
