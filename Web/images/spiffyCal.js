...
img_Del.src="jscript48/btn_del_small_gif";
img_Close.src="jscript48/btn_close_small_gif";
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	var dteCur=new Date(dteToday.getFullYear()-40, dteToday.getMonth(), dteToday.getDate());
	...
	//var strDefBtnImgPath=calMgr.DefBtnImgPath;
        var strDefBtnImgPath='/ORS/Web/images/';
	/* PROPERTIES =============================================================
	 *
	 */
	// Generic Properties
	...
	this.minYearChoice=dteToday.getFullYear()-40;	
	this.maxYearChoice=dteToday.getFullYear()-20;
	...
	function writeControl() {
		...
		if (!(this.useDateRange)) { 
			...
			strTemp+=' onChange="calMgr.validateDate('+'document.'+this.formName+'.'+this.textBoxName+','+this.varName+'.required,'+quote(strTempMinDate)+','+quote(strTempMaxDate)+');" onBlur="calMgr.formatDate(document.'+this.formName+'.'+this.textBoxName+','+this.varName+'.dateFormat);" ';
                ...
                strHold='<input class="cal-TextBox" type="text" readonly name="' + this.textBoxName + '"' + strTemp + 'size="12" value="' + this.getSelectedDate() + '">';
		if (!scIE) {
